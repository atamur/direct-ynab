...
class TestFixtureVersionIsolation:
    ...
    @budget_version(67)
    def test_fixture_only_applies_deltas_up_to_target_version(
        self, version_aware_parser
    ):
        ...
        for delta_file in parser.applied_deltas:
            ...
            assert (
                end_version <= 67
            ), f"Delta {delta_file.name} has end version {end_version} > 67"
    ...
    def test_fixture_without_version_annotation_applies_all_deltas(
        self, version_aware_parser
    ):
